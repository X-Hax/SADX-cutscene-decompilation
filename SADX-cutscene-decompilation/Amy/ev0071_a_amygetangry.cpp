...
void ev0071_a_amygetangry(int state)
{
	...
	case 1:
		...
		EV_SetAng(player, 63488, 61440, 0);
		...
		EV_SetAng(obj_wing2, 64768, 27392, 0);
		...
		EV_CameraAng(0, 80, 4227, 1782, 256);
		EV_CameraPerspective(1, 80, 10377);
		EV_Wait(85);
		EV_ClrFace(player);
		EV_ClrAction(player);
		...
		EV_SetAng(zero, 65024, 14640, 0);
		EV_CameraPos(1, 0, -10.4f, 747.70001f, -264.70001f);
		EV_CameraAng(1, 0, 2816, 60320, 512);
		EV_CameraPos(1, 120, -4.3000002f, 757.40002f, -293.5f);
		EV_CameraAng(1, 120, 3328, 63392, 512);
		moveObjectAngle2(
			obj_wing2,
		...
		EV_SetAng(player, 65024, 14640, 0);
		EV_ClrAction(player);
		...
		EV_CameraAng(1, 0, 3840, 2208, 512);
		EV_SerifPlay(987);
		...
		EV_CameraAng(0, 0, 61512, 56406, 0);
		EV_CameraPos(0, 50, -7.9000001f, 755.78998f, -280.79999f);
		EV_CameraAng(0, 50, 62536, 60246, 0);
		...
		EV_CameraAng(0, 30, 3656, 51286, 2304);
		EV_Wait(30);
		...
		EV_CameraAng(0, 100, 2376, 17494, 2048);
		EV_Wait(110);
		EV_ClrFace(player);
		EV_MsgClose();
		break;
