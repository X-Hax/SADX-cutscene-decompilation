...
void ev0064_a_kidnapped(int state)
{
	...
	case 1:
		...
		EV_SetAng(obj_wing2, 0, 0xC000, 0);
		EV_Wait(1);
		moveObjectOn(obj_wing2, 2.5f, 6.8000002f, -3.5f, 200, player);
		EV_CreatePlayer(3, EggrobForEvent0, 395.10001f, 0.0f, 1351.0f, 0, 0xC000, 0);
		EV_Wait(1);
		zero = EV_GetPlayer(3);
		...
		EV_CameraAng(1, 0, 0xF00, 0x8534, 0);
		...
		EV_CameraAng(1, 120, 0xE49C, 0xF586, 0xF500);
		...
		EV_CameraAng(0, 0, 0x189C, 0x7186, 0xFC00);
		EV_CameraPos(0, 16, 341.60001f, 0.69999999f, 1329.0f);
		EV_CameraAng(0, 16, 0xF9C, 0x7186, 0xFA00);
		EV_Wait(30);
		...
		EV_CameraAng(0, 0, 0x69C, 0x7F86, 0);
		EV_SetPos(obj_wing2,
			player->twp->pos.x,
		...
		EV_CameraAng(0, 80, 0x39C, 0x7F86, 0);
		...
		EV_CameraAng(0, 25, 0x49C, 0x7F86, 0);
		EV_Wait(30);
		EV_ClrFace(player);
		EV_RemovePlayer(3);
		EV_FreeObject(&obj_wing2);
		EV_CreateObjectFunc(&egg_amy, set_amy, 330.0f, 11.0f, 1205.9f, 0, 0xD000, 0);
		EV_Wait(1);
		EV_SetMode(egg_amy, 0);
		setamyparam(1.1f);
		EV_SetPos(egg_amy, 330.0f, 11.0f, 1205.9f);
		EV_SetAng(egg_amy, 0, 0x9000, 0);
		EV_CameraPerspective(0, 1, 14746);
		EV_Wait(1);
		EV_CameraPos(0, 0, 323.04999f, 24.1f, 1187.86f);
		EV_CameraAng(0, 0, 0x1211, 0xAE2C, 0);
		EV_CameraPos(0, 60, 323.5f, 14.1f, 1198.0f);
		EV_CameraAng(0, 60, 0x1211, 0xAE2C, 0);
		EV_Wait(70);
		EV_MsgClose();
		EV_CameraPerspective(0, 1, 13289);
		EV_Wait(1);
		EV_CameraPos(0, 0, 324.5f, 17.299999f, 1177.2f);
		EV_CameraAng(0, 0, 0x11, 0x882C, 0xFF00);
		EV_CameraPos(0, 30, 323.5f, 4.4000001f, 1170.6f);
		EV_CameraAng(0, 30, 0x1411, 0x882C, 0xFF00);
		EV_Wait(70);
		EV_CameraPos(1, 0, 143.2f, 4.0f, 1644.6f);
		EV_CameraAng(1, 0, 0xA00, 0x7F4, 0);
		EV_CameraTargetFree();
		EV_CameraPos(1, 70, 197.3f, 3.8f, 1622.1f);
		EV_CameraAng(1, 70, 0x500, 0x1500, 0);
		moveObjectAngle2(egg_amy, 192.60001f, 16.0f, 1617.2f, 71.5f, 21.5f, 1461.4f, 0, 0xD000, 0, 0, 0xD000, 0, 140);
		EV_SerifPlay(882);
		EV_Msg(msgTbl_ev0064[TextLanguage][3]); //"\aHey, watch it bolt brain, \nlet us do"...
		EV_CameraPos(1, 120, 161.5f, 4.8000002f, 1577.1f);
		EV_CameraAng(1, 120, 0x500, 0x1800, 0);
		EV_SerifPlay(883);
		EV_Wait(60);
		moveObjectAngle2(egg_amy, 71.5f, 21.5f, 1461.4f, 48.5f, 35.799999f, 1398.4f, 0, 0xD000, 0, 0, 0xD000, 0, 75);
		EV_Wait(75);
		stopObjectAll();
		break;
	case 2:
